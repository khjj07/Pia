...
public class PressurePlate : InteractableClass
{
    ...
    {
        if (Player.UpperState.Dagger == state && catchButton.isPressed)
        {
    ...
    public override void OnInteract(object player)
    {
        if (catchButton.isPressed)
        {
            ...
            Observable.Interval(TimeSpan.FromSeconds(0.01f))
                ...
                .TakeWhile(_ => count < targetCount)
                .Subscribe(_ => SetMatchBarAngle(_matchBarAngle + _currentSpeed));
    ...
    private bool CheckMatchBarInBound()
    {
